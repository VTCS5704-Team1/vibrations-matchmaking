...
def find_matches_using_cosine_similarity(user, user_list, threshold=0.5):
    user_feature_vector = create_feature_vector([user])[0]
    for potential_match in user_list:
        if user != potential_match:  # Avoid matching with oneself
            potential_match_feature_vector = create_feature_vector([potential_match])[0]
            similarity_score = calculate_cosine_similarity(user_feature_vector, potential_match_feature_vector)
