...
def generate_user_profiles(num_profiles):
    ...
    for i in range(1, num_profiles + 1):
        ...
        fav_songs = [f"Song{j}" for j in range(1, 500)]
        fav_artists = [f"Artist{j}" for j in range(1, 500)]
        latitude = random.uniform(-90, 90)
        longitude = random.uniform(-180, 180)
        max_dist = random.uniform(2000, 5000)

        user = User(user_id, first_name, last_name, profile_picture_link, email, phone_no, fav_songs, fav_artists, latitude, longitude, max_dist)
